(() => {
  ...
  function wireCartDrawer(products, config, formatter) {
    ...
    function renderCart() {
      ...
      cart.items.forEach((it) => {
        ...
        img.loading = "lazy";
        img.src = product.images?.[0] || "assets/store/product-01.svg";
  ...
  document.addEventListener("DOMContentLoaded", () => {
    ...
    const slider = createSlider({
      ...
      images,
      alt: product.name,
      onOpen: (src, altText) => zoom.open(src, altText),
...
// منع القفل لما تدوس على الصورة نفسها
...
overlay.addEventListener("wheel", (e) => {
  ...
  scale = Math.min(Math.max(1, scale), 4); // بين 1 و 4
...
overlay.addEventListener("touchmove", (e) => {
  ...
  // swipe (drag بإصبع واحد)
  if (e.touches.length === 1) {
    translateX += e.touches[0].movementX || 0;
    translateY += e.touches[0].movementY || 0;
    updateTransform();
  }
});
